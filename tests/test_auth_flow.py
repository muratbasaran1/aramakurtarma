...
def parse_datetime(value: str) -> datetime:
    """Parse ISO-8601 strings with optional Z suffix into aware datetimes."""
...
async def test_superuser_bootstrap_and_incident_flow(client):
    ...
    incident_reported_at = parse_datetime(incident_data["reported_at"])
    incident_updated_at = parse_datetime(incident_data["updated_at"])
    assert incident_updated_at >= incident_reported_at
    assert incident_updated_at - incident_reported_at < timedelta(seconds=1)
    assert incident_updated_at.tzinfo == timezone.utc
    ...
    high_severity_list = await client.get(
        "/api/v1/incidents/",
    ...
    assert high_severity_list.status_code == 200
    assert len(high_severity_list.json()) == 1

    low_severity_list = await client.get(
        "/api/v1/incidents/",
        params={"severity": "low"},
        headers={
            "Authorization": f"Bearer {tenant_token}",
    ...
    assert low_severity_list.status_code == 200
    assert len(low_severity_list.json()) == 0
    ...
    updated_incident_data = updated_incident.json()
    assert updated_incident_data["status"] == update_payload["status"]
    latest_incident_updated_at = parse_datetime(updated_incident_data["updated_at"])
    assert latest_incident_updated_at > incident_updated_at
    incident_updated_at = latest_incident_updated_at
    ...
    in_progress_incidents = await client.get(
        "/api/v1/incidents/",
    ...
    assert in_progress_incidents.status_code == 200
    assert len(in_progress_incidents.json()) == 1

    closed_incidents = await client.get(
        "/api/v1/incidents/",
    ...
    assert closed_incidents.status_code == 200
    assert len(closed_incidents.json()) == 0

    multi_status_incidents = await client.get(
        "/api/v1/incidents/",
        params=[("status", "in_progress"), ("status", "closed")],
        headers={
            "Authorization": f"Bearer {tenant_token}",
            "X-Tenant-ID": "ankara-afad",
        },
    )
    assert multi_status_incidents.status_code == 200
    assert len(multi_status_incidents.json()) == 1

    combined_filter_incidents = await client.get(
        "/api/v1/incidents/",
        params={"status": "in_progress", "severity": "high"},
        headers={
            "Authorization": f"Bearer {tenant_token}",
            "X-Tenant-ID": "ankara-afad",
        },
    )
    assert combined_filter_incidents.status_code == 200
    assert len(combined_filter_incidents.json()) == 1

    mismatched_combination = await client.get(
        "/api/v1/incidents/",
        params={"status": "in_progress", "severity": "low"},
        headers={
            "Authorization": f"Bearer {tenant_token}",
            "X-Tenant-ID": "ankara-afad",
        },
    )
    assert mismatched_combination.status_code == 200
    assert len(mismatched_combination.json()) == 0
    ...
    request_created_at = parse_datetime(request_data["created_at"])
    request_updated_at = parse_datetime(request_data["updated_at"])
    assert request_updated_at >= request_created_at
    assert request_updated_at - request_created_at < timedelta(seconds=1)
    ...
    updated_request_timestamp = parse_datetime(updated_request["updated_at"])
    assert updated_request_timestamp > request_updated_at
    request_updated_at = updated_request_timestamp
    ...
    task_created_at = parse_datetime(task_data["created_at"])
    task_updated_at = parse_datetime(task_data["updated_at"])
    assert task_created_at.tzinfo == timezone.utc
    assert task_updated_at >= task_created_at
    assert task_updated_at - task_created_at < timedelta(seconds=1)
    ...
    new_task_updated_at = parse_datetime(updated_task["updated_at"])
    assert new_task_updated_at > task_updated_at
    task_updated_at = new_task_updated_at
    ...
    assert task_detail_data["status"] == "in_progress"
    detail_created_at = parse_datetime(task_detail_data["created_at"])
    assert detail_created_at == task_created_at
    detail_updated_at = parse_datetime(task_detail_data["updated_at"])
    assert detail_updated_at == task_updated_at
    ...
    deployment_created_at = parse_datetime(deployment_data["created_at"])
    deployment_updated_at = parse_datetime(deployment_data["updated_at"])
    assert deployment_updated_at >= deployment_created_at
    assert deployment_updated_at - deployment_created_at < timedelta(seconds=1)
    ...
    deployment_detail_data = deployment_detail.json()
    assert deployment_detail_data["leader_id"] == str(new_user_id)
    detail_updated_at = parse_datetime(deployment_detail_data["updated_at"])
    assert detail_updated_at == deployment_updated_at
    ...
    new_deployment_updated_at = parse_datetime(updated_deployment["updated_at"])
    assert new_deployment_updated_at > deployment_updated_at
    deployment_updated_at = new_deployment_updated_at
    ...
    report_created_at = parse_datetime(report_data["created_at"])
    report_updated_at = parse_datetime(report_data["updated_at"])
    assert report_updated_at >= report_created_at
    assert report_updated_at - report_created_at < timedelta(seconds=1)
    ...
    report_detail_data = report_detail.json()
    assert report_detail_data["deployment_id"] == deployment_data["id"]
    detail_updated_at = parse_datetime(report_detail_data["updated_at"])
    assert detail_updated_at == report_updated_at
    ...
    new_report_updated_at = parse_datetime(updated_report["updated_at"])
    assert new_report_updated_at > report_updated_at
    report_updated_at = new_report_updated_at
    ...
    delivery_created_at = parse_datetime(delivery_data["created_at"])
    delivery_updated_at = parse_datetime(delivery_data["updated_at"])
    assert delivery_updated_at >= delivery_created_at
    assert delivery_updated_at - delivery_created_at < timedelta(seconds=1)
    ...
    delivery_detail_data = delivery_detail.json()
    assert delivery_detail_data["deployment_id"] == deployment_data["id"]
    detail_updated_at = parse_datetime(delivery_detail_data["updated_at"])
    assert detail_updated_at == delivery_updated_at
    ...
    new_delivery_updated_at = parse_datetime(updated_delivery["updated_at"])
    assert new_delivery_updated_at > delivery_updated_at
    delivery_updated_at = new_delivery_updated_at
    ...
    plan_created_at = parse_datetime(plan_data["created_at"])
    plan_updated_at = parse_datetime(plan_data["updated_at"])
    assert plan_updated_at >= plan_created_at
    assert plan_updated_at - plan_created_at < timedelta(seconds=1)
    ...
    plan_detail_data = plan_detail.json()
    assert plan_detail_data["title"] == plan_payload["title"]
    detail_updated_at = parse_datetime(plan_detail_data["updated_at"])
    assert detail_updated_at == plan_updated_at
    ...
    returned_completion = parse_datetime(updated_plan["completed_at"])
    assert returned_completion == completion_time
    assert updated_plan["priority"] == "medium"
    new_plan_updated_at = parse_datetime(updated_plan["updated_at"])
    assert new_plan_updated_at > plan_updated_at
    plan_updated_at = new_plan_updated_at
