...
| Audit Log Kaydı & API | `backend/app/Support/Audit/`, `backend/app/Models/AuditLog.php`, `backend/app/Http/Controllers/Api/AuditLogController.php`, `backend/app/Http/Resources/AuditLogResource.php`, `backend/database/migrations/0001_01_01_110000_create_audit_logs_table.php` | Tenant API’lerinde gerçekleşen oluşturma/güncelleme/silme işlemlerini maskeleme ile kayıt altına alır ve `GET /api/tenants/{tenant}/audit-logs` ucu üzerinden sorgulanabilir hale getirir. | Faz 1, 3, 11 |
| Canlı Takip & Hareketsizlik | `backend/app/Support/Tracking/`, `backend/app/Http/Controllers/Api/TrackingPingController.php`, `backend/app/Http/Requests/Api/Tracking/`, `backend/app/Http/Resources/TrackingPingResource.php`, `backend/database/migrations/0001_01_01_120000_create_tracking_pings_table.php` | GPS ping kayıtlarını tenant doğrulamasıyla alır, `StoreTrackingPingRequest` aktif görev (`assigned/in_progress`) ve kullanıcı/birim eşleşmesi olmayan pingleri 422 ile reddeder; hareketsizlik kuralını (120 sn / 5 m / 0.3 m/s) değerlendiren `MotionMonitor` audit log üretir, `tracking/pings/latest` uç noktası her kullanıcı için güncel ping’i OpsCenter akışına besler. | Faz 4, 7 |
| Audit Log Kaydı | `backend/app/Support/Audit/`, `backend/app/Models/AuditLog.php`, `backend/database/migrations/0001_01_01_110000_create_audit_logs_table.php` | Tenant API’lerinde gerçekleşen oluşturma/güncelleme/silme işlemlerini maskeleme ile kayıt altına alır; denetim izleri Faz 1 ve Faz 11 hedefleriyle uyumludur. | Faz 1, 3, 11 |
...
| Kullanıcı Yönetimi API’si | `backend/app/Http/Controllers/Api/UserController.php`, `backend/app/Http/Requests/Api/Users/`, `backend/tests/Feature/Api/UserApiTest.php` | Tenant izolasyonlu kullanıcı listeleme, detay, oluşturma, güncelleme ve aktif görevi olmayan kayıtlar için silme uçlarını belge doğrulaması ve şifre politikalarıyla sağlar. | Faz 1, 3 |
...
| Tenant Yönetim API’si | `backend/app/Http/Requests/Api/Tenants/`, `backend/app/Http/Controllers/Api/TenantController.php`, `backend/tests/Feature/Api/TenantApiTest.php` | Tenant oluşturma/güncelleme/silme akışlarını slug/timezone doğrulaması ve bağımlılık kontrolleriyle standartlaştırır, OpsCenter özetini isteğe bağlı döndürür. | Faz 2, 3, 6, 11 |
...
- `backend/app/Support/Audit/AuditLogger.php` API’deki `create/update/delete` işlemlerini `audit_logs` tablosuna taşıyarak tenant kapsamı ve hassas veri maskelemesini otomatikleştirir; denetim kayıtları Faz 1 güvenlik politikalarına uygun şekilde tutulur.
